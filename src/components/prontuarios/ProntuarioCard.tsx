...
import { User, Calendar, Activity, ArrowRight, AlertTriangle, Trash2 } from 'lucide-react'; // Adicionado Trash2
...
// 2. Adicionado onDelete na props
...
export default function ProntuarioCard({ prontuario, index = 0, onDelete }: ProntuarioCardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, delay: index * 0.05 }}
      className="group bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 hover:shadow-xl hover:shadow-blue-500/10 hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-300 relative"
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white text-lg pr-8">
        ...
        {prontuario.prioridade === 'Urgente' && (
          <div className="flex items-center gap-1 text-red-500 animate-pulse mr-8">
      ...
      <div className="space-y-3 mb-5">
        <div>
          ...
          <p className="text-slate-800 dark:text-slate-200 font-medium line-clamp-1">{prontuario.diagnostico_principal}</p>
        </div>
